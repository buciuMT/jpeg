...
@dataclass
class Hnode:
    def __init__(self,count:int,char:np.int8):
        self.count:int=count
        self.char:np.int8=char

def generate_huffman_tree(arr:NDArray[np.int8]):
    heap=[HuffmanNode(char=np.int8(element),count=count) for element,count in Counter(arr).items()]
    heapq.heapify(heap)
    while len(heap)>1:
...
@functools.total_ordering #https://en.wikibooks.org/wiki/JPEG_-_Idea_and_Practice/The_Huffman_coding
class HuffmanNode:
    def __init__(self,char:None|np.int8=None,count:int=-1,l:None|Self=None,r:None|Self=None):
        self.char:None|np.int8=None
        self.count:int=count
        self.left:Self|None=l
        self.right:Self|None=r


    def getbits(self)->dict[np.int8,bitstring.Bits]:
        if self.char is not None:
            return {self.char:bitstring.Bits()}
        if self.left is None or self.right is None:return {}
        return {c:(b+False) for (c,b) in self.left.getbits()}|{c:(b+True) for (c,b) in self.left.getbits()}
...
DIAG_ORDER=np.array([ 0,  1,  8, 16,  9,  2,  3, 10, 17, 24, 32, 25, 18, 11,  4,  5, 12,
...
def rgb2ycbcr(img:NDArray[np.uint8])->NDArray[np.uint8]:
    shape=img.shape
    return np.clip(img.reshape(-1,3)@RGB_TO_YCBCR_MULT.T+RGB_TO_YCBCR_ADD,0,255).reshape(shape).astype(np.uint8)

def ycbcr2rgb(img:NDArray[np.uint8])->NDArray[np.uint8]:
    shape=img.shape
    return np.clip((img.reshape(-1,3)+YCBCR_TO_RGB_ADD.T)@YCBCR_TO_RGB_MULT.T,0,255).reshape(shape).astype(np.uint8)
...
def encode_jpeg(image,args:argparse.Namespace,qlevel:None|int=None):
    ...
    hy=generate_huffman_tree(qby.astype(np.int8).flatten())
    print(hy.getbits())
...
def main():
    ...
    _=arg_parser.add_argument('-i','--input',type=str,required=True,help="input file")
